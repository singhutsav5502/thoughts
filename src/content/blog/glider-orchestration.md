...
*why not make the harness own the mission shape, not just which model answers?*
...
| **HITL / human gate** | A pause where a person must approve before the job continues. |
...
You don't need a hoop graph on day one. But once agents touch real work, picking a model stops being the product — the *mission shape* is: who runs, in what order, what they share, and when a human gets a veto.
